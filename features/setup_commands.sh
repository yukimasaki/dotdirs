...
# /**
#  * @file setup_commands.sh
#  * @description コマンドファイルをエディタ・エージェント別の所定ディレクトリにコピーする。
#  * サブディレクトリ構造を維持したままコピーする。
#  */
...
# /**
#  * @function copy_commands_with_structure
#  * @description コマンドファイルをサブディレクトリ構造を維持したままコピーする純粋関数。
#  * @param {string} source_dir - ソースディレクトリ（commandsディレクトリのパス）
#  * @param {string} target_base_dir - ターゲットベースディレクトリ（例: $HOME/.cursor）
#  * @param {string} target_subdir - ターゲットサブディレクトリ（例: commands）
#  * @param {string} source_ext - ソースファイル拡張子（例: md）
#  * @param {string} target_ext - ターゲットファイル拡張子（例: md）
#  * @returns {number} 成功時は0、失敗時は1
#  */
copy_commands_with_structure() {
    local source_dir="$1"
...
select_target_directory() {
    ...
    gum style \
        ...
        "Commands Setup" "Select target directory" >&2
...
#  * @description コマンドファイルをセットアップする（TUI対応）。
...
setup_commands() {
    ...
    # コマンドディレクトリはSCRIPT_DIR（dotdirs）にある
    local commands_source_dir="${script_dir}/storage/__default__/dynamic/commands"
    
    # コマンドディレクトリの存在確認
    if [ ! -d "$commands_source_dir" ]; then
        echo "Error: Commands directory not found at $commands_source_dir" >&2
        return 1
    fi
    ...
    gum style \
        --foreground 212 --border-foreground 212 --border double \
        --align center --width 50 --margin "1 2" --padding "2 4" \
        "Commands Setup" "Select editor/agent" >&2
    ...
    gum style \
        --foreground 212 --border-foreground 212 --border double \
        --align center --width 50 --margin "1 2" --padding "2 4" \
        "Copying Commands" "Copying command files with directory structure..."
    
    if copy_commands_with_structure "$commands_source_dir" "$target_base_dir" "$target_subdir" "md" "$target_ext"; then
        echo ""
        gum style \
            --foreground 46 --border-foreground 46 --border double \
            --align center --width 50 --margin "1 2" --padding "2 4" \
            "Success" "Command files copied to:\n${target_dir}"
        return 0
    else
        echo ""
        gum style \
            ...
            --align center --width 50 --margin "1 2" --padding "2 4" \
            "Error" "Failed to copy command files"
        return 1
    fi
}
